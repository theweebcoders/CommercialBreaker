...
class BasePage(gui.Container):
    def __init__(self, app, title_key, *args, **kwargs):
        ...
        self.add_page_title(
            self.main_container, self.app.page_titles.get(title_key, "")
        )
    ...
    def refresh_nav_bar(self):
        ...
        main_container = (
            self.main_container if hasattr(self, "main_container") else None
        )
    ...
    def create_status_bar(self):
        ...
        self.status_progress_container = gui.Container(
            style=Styles.status_progress_container_style
        )

        # Create the progress fill element
        self.status_progress_fill = gui.Container(
            style=Styles.status_progress_fill_style
        )

        # Create percentage text element
        self.status_progress_text = gui.Label(
            "0%", style=Styles.status_progress_text_style
        )
    ...
    def update_status_display(
        self, status_text, show_progress=False, progress_value=None
    ):
    ...
    def add_input(self, container, default_value="", input_type="text"):
        input_widget = gui.Input(
            input_type=input_type, style=Styles.default_input_style
        )
    ...
    def add_labeled_input(
        self, container, label_text, default_value="", input_type="text"
    ):
    ...
    def add_button(self, container, text, onclick_handler):
        button = gui.Button(
            text, width=200, height=30, style=Styles.default_button_style
        )
        button.onclick.do(onclick_handler)
        container.append(button)
        return button

    def add_button_with_style(
        self, container, text, onclick_handler, style_type="primary"
    ):
        ...
        """ % (
            ...
            ";".join(
                [
                    f"btn.style.{k.replace('-', '')} = '{v}'"
                    for k, v in hover_style.items()
                ]
            ),
            ";".join(
                [
    ...
    def add_checkbox(self, container, label_text, default_value=False):
        hbox = gui.HBox(
            style={"align-items": "center", "margin": "5px", **Styles.transparent_style}
        )
...
class NavigationBar(gui.Container):
    def __init__(self, app, current_page, *args, **kwargs):
        ...
        back_button = self.add_button(
            left_container, "← Back", self.on_back_button_click
        )
        ...
        home_button = self.add_button(
            right_container, "↻ Start Over", self.on_home_button_click
        )
        # Apply navigation style manually
        home_button.style.update(Styles.navigation_button_style)

        # Center container for progress indicators
        center_container = gui.HBox(
            style={
                "align-items": "center",
                "justify-content": "center",
                "flex-grow": "1",
    ...
    def create_page_indicators(self, container):
        ...
        # Always add Page1
        pages.append(
            {"id": "Page1", "title": "Login", "optional": False, "visual_num": 1}
        )
        ...
        for i, page in enumerate(pages):
            ...
            indicator_style = {
                ...
                "box-shadow": (
                    "0 0 8px rgba(0, 255, 255, 0.8)" if is_current else "none"
                ),
                "cursor": "pointer",
            }
...
class Page1(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        self.app.execute_javascript(
            ...
            window.pywebview.api.open_jellyfin_auth_url = function(url) {
                window.open(url, '_blank');
            }
        """
        )
        ...
        self.logic.subscribe_to_jellyfin_servers(self.handle_jellyfin_servers_update)
        self.logic.subscribe_to_jellyfin_libraries(
            self.handle_jellyfin_libraries_update
        )
        self.logic.subscribe_to_plex_auth_url(self.handle_plex_auth_url)
        self.logic.subscribe_to_jellyfin_auth_url(self.handle_jellyfin_auth_url)
        self.logic.subscribe_to_media_server_type(self.handle_media_server_type_update)
        ...
        # Media Server Login Section
        self.add_label(self.main_container, "Choose Media Server")

        # Create container for media server login buttons
        login_container = gui.HBox(
            style={
                "justify-content": "center",
                "margin": "10px 0",
                "gap": "20px",
        ...
        self.login_with_plex_button = self.add_button_with_style(
            login_container, "Login with Plex", self.login_to_plex, "primary"
        )
        self.login_with_jellyfin_button = self.add_button_with_style(
            login_container, "Login with Jellyfin", self.login_to_jellyfin, "secondary"
        )

        self.main_container.append(login_container)

        # Server URL Entry for Jellyfin (initially hidden)
        self.jellyfin_url_container = gui.Container(style={"display": "none"})
        self.add_label(self.jellyfin_url_container, "Jellyfin Server URL:")
        self.jellyfin_url_entry = self.add_input(
            self.jellyfin_url_container, "e.g., http://localhost:8096"
        )
        self.main_container.append(self.jellyfin_url_container)

        # Servers Dropdown (unified for both Plex and Jellyfin)
        self.add_label(self.main_container, "Select a Server")
        self.server_dropdown = self.add_dropdown(
            self.main_container, ["Select a Server"], self.on_server_selected
        )

        # Anime Library Dropdown
        self.add_label(self.main_container, "Select your Anime Library")
        self.anime_library_dropdown = self.add_dropdown(
            self.main_container,
            ["Select your Anime Library"],
            self.add_1_to_libraries_selected,
        )

        # Toonami Library Dropdown
        self.add_label(self.main_container, "Select your Toonami Library")
        self.library_dropdown = self.add_dropdown(
            self.main_container,
            ["Select your Toonami Library"],
            self.add_1_to_libraries_selected,
        )

        # Platform Selection
        platform_container = gui.HBox(
            style={
                "justify-content": "center",
                "margin": "20px",
                "gap": "20px",
        ...
        self.dizquetv_button = gui.Button(
            "DizqueTV", width=150, height=40, style=Styles.selected_button_style
        )
        self.tunarr_button = gui.Button(
            "Tunarr", width=150, height=40, style=Styles.unselected_button_style
        )
        ...
        self.platform_url_entry = self.add_input(
            self.main_container, "e.g., http://localhost:17685"
        )

        # Remove individual status label since we now use the global one in BasePage

        # Continue and Skip buttons
        buttons_container = gui.HBox(
            style={
                "justify-content": "center",
                "margin-top": "20px",
                "background": "transparent",
        ...
        self.continue_button = self.add_button_with_style(
            buttons_container, "Continue", self.on_continue_button_click, "secondary"
        )
        self.continue_button.style["display"] = "none"
        buttons_container.append(self.continue_button)

        self.skip_button = self.add_button_with_style(
            buttons_container, "Skip", self.on_skip_button_click, "secondary"
        )
    ...
    def login_to_jellyfin(self, widget):
        print("You pressed the Jellyfin login button")
        self.media_server_type = "jellyfin"

        # Show Jellyfin URL entry if hidden
        self.jellyfin_url_container.style["display"] = "block"

        # Get server URL from input field
        server_url = self.jellyfin_url_entry.get_value()
        if not server_url or server_url.startswith("e.g.,"):
            self.update_status_display(
                "Please enter a valid Jellyfin server URL first."
            )
            return

        self.logic.login_to_jellyfin(server_url)
        self.update_status_display("Logging into Jellyfin...")

    def on_server_selected(self, widget, value):
        self.selected_server = self.server_dropdown.get_value()
    ...
    def update_server_dropdown(self, server_list):
        ...
        first_item = self.server_dropdown.children.get("0")
        self.server_dropdown.empty()
        if first_item:
            self.server_dropdown.append(first_item)

        # Add all servers from the provided list
        if server_list:
            for server in server_list:
                self.server_dropdown.append(gui.DropDownItem(server))
            ...
            if len(server_list) == 1:
                ...
                self.server_dropdown.set_value(server_name)
    ...
    def update_library_dropdowns(self, library_list):
        ...
        first_anime_item = self.anime_library_dropdown.children.get("0")
        first_toonami_item = self.library_dropdown.children.get("0")

        self.anime_library_dropdown.empty()
        self.library_dropdown.empty()

        # Add back the first instruction items
        if first_anime_item:
            self.anime_library_dropdown.append(first_anime_item)
        if first_toonami_item:
            self.library_dropdown.append(first_toonami_item)

        # Add all libraries to both dropdowns
        if library_list:
            for library in library_list:
                self.anime_library_dropdown.append(gui.DropDownItem(library))
                self.library_dropdown.append(gui.DropDownItem(library))
    ...
    def on_continue_button_click(self, widget):
        selected_anime_library = self.anime_library_dropdown.get_value()
        selected_toonami_library = self.library_dropdown.get_value()
    ...
    def handle_jellyfin_auth_url(self, auth_url):
        """Handle Jellyfin authentication URL - open in browser"""
        print(f"Received Jellyfin auth URL: {auth_url}")
        self.app.execute_javascript(f"window.open('{auth_url}', '_blank')")
        print(f"Opening Jellyfin auth URL: {auth_url}")

    def handle_jellyfin_servers_update(self, server_list_json):
        """Handle Jellyfin servers list update"""
        try:
            # Deserialize the JSON string to a Python list
            server_list = json.loads(server_list_json)
            print(f"Received Jellyfin server list with {len(server_list)} items")
            ...
            print(f"Error processing Jellyfin server list: {e}")

    def handle_jellyfin_libraries_update(self, library_list_json):
        """Handle Jellyfin libraries list update"""
        try:
            # Deserialize the JSON string to a Python list
            library_list = json.loads(library_list_json)
            print(f"Received Jellyfin library list with {len(library_list)} items")
            ...
            # Update status
            self.update_status_display(f"Loaded {len(library_list)} Jellyfin libraries")
        except Exception as e:
            print(f"Error processing Jellyfin library list: {e}")
    ...
    def handle_media_server_type_update(self, server_type):
        ...
        try:
            ...
            if server_type == "jellyfin":
                ...
                self.dizquetv_button.style.update(Styles.disabled_button_style)
                self.dizquetv_button.attributes["disabled"] = "true"

                # Auto-select Tunarr
                if self.selected_platform == "dizquetv":
                    self.on_platform_change("tunarr")

                self.update_status_display(
                    "DizqueTV disabled: not compatible with Jellyfin. Using Tunarr."
                )
            else:
                # Re-enable DizqueTV for Plex
                self.dizquetv_button.style.update(Styles.unselected_button_style)
                if "disabled" in self.dizquetv_button.attributes:
                    del self.dizquetv_button.attributes["disabled"]
...
class Page2(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        # Plex URL Entry
        self.plex_url_entry = self.add_labeled_input(
            self.main_container, "Plex URL:", "e.g., http://localhost:32400"
        )

        # Plex Token Entry
        self.plex_token_entry = self.add_labeled_input(
            self.main_container, "Plex Token:", "e.g., xxxxxxxxxxxxxx"
        )

        # Plex Anime Library Entry
        self.plex_anime_library_entry = self.add_labeled_input(
            self.main_container, "Plex Anime Library:", "e.g., Anime"
        )

        # Plex Toonami Library Entry
        self.plex_toonami_library_entry = self.add_labeled_input(
            self.main_container, "Plex Toonami Library:", "e.g., Toonami"
        )
        ...
        # Platform URL Entry
        self.platform_url_entry = self.add_labeled_input(
            self.main_container, "Platform URL:", "e.g., http://localhost:17685"
        )

        # Continue button
        self.continue_button = self.add_button_with_style(
            self.main_container, "Continue", self.on_continue_button_click, "secondary"
        )
...
class Page3(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        self.prepare_button = self.add_button_with_style(
            self.main_container, "Prepare Content", self.prepare_content, "primary"
        )

        # Get Plex Timestamps button
        self.add_label(self.main_container, "Get Plex Timestamps")
        self.get_plex_timestamps_button = self.add_button_with_style(
            self.main_container,
            "Get Plex Timestamps",
            self.get_plex_timestamps,
            "primary",
        )
        ...
        filter_mode_container = gui.Container(
            style={
                "width": "100%",
                "display": "flex",
                "justify-content": "center",
        ...
        self.prepopulate_button = gui.Button(
            "Prepopulate Selection", width=200, height=40
        )
        self.prepopulate_button.style.update(Styles.unselected_button_style)
        self.prepopulate_button.onclick.do(
            lambda w: self.set_filter_mode("prepopulate")
        )

        # Add buttons to the row container with spacing
        buttons_row.append(self.move_files_button)
        buttons_row.append(
            gui.Label("", style={"width": "20px", "background": "transparent"})
        )  # Spacer
        ...
        process_button_container = gui.Container(
            style={
                "width": "100%",
                "display": "flex",
                "justify-content": "center",
        ...
        self.move_filtered_shows_button = self.add_button_with_style(
            process_button_container,
            "Process Filtered Shows",
            self.move_filtered,
            "primary",
        )
    ...
    def prepare_content(self, widget):
        ...
        for i in range(25):
            try:
                ...
                self.display_show_selection(
                    unique_show_names, easy_checker, toonami_episodes
                )
            ...
            self.logic._broadcast_status_update(
                "Prepare content failed. Please try again."
            )
    ...
    def display_show_selection(self, unique_show_names, easy_checker, toonami_episodes):
        ...
        # Add a header
        header = gui.Label(
            "Select shows to include in your lineup:",
            style={
                "font-size": "18px",
                "font-weight": "bold",
                "color": "#a5f3fc",
        ...
        helper_text = gui.Label(
            "All shows are selected by default. Uncheck any shows you want to exclude.",
            style={
                "font-size": "14px",
                "color": "#a5f3fc",
                "margin-bottom": "15px",
        ...
        select_all_button = self.add_button_with_style(
            buttons_container,
            "Select All",
            lambda w: self.toggle_all_checkboxes(True),
            "secondary",
        )

        deselect_all_button = self.add_button_with_style(
            buttons_container,
            "Deselect All",
            lambda w: self.toggle_all_checkboxes(False),
            "secondary",
        )

        done_button = self.add_button_with_style(
            buttons_container,
            "Done",
    ...
    def on_done_button_clicked(self, widget, easy_checker, toonami_episodes):
        selected_shows = [
            show for show, checkbox in self.checkboxes.items() if checkbox.get_value()
        ]
        for i in range(25):
            try:
...
class Page4(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        # === FOLDER MODE UI ===
        self.folder_mode_container = gui.VBox(
            style={
                "align-items": "center",
                "justify-content": "flex-start",
                "width": "100%",
        ...
        self.input_path_input = self.add_labeled_input(
            self.folder_mode_container, "Input directory:", self.default_input_folder
        )
        ...
        self.file_mode_container = gui.VBox(
            style={
                "align-items": "center",
                "justify-content": "flex-start",
                "width": "100%",
        ...
        file_list_container = gui.Container(
            style={
                "width": "90%",
                "height": "200px",
                "margin": "10px",
        ...
        self.file_listbox = gui.ListView(
            style={
                "width": "100%",
                "height": "100%",
        ...
        self.output_path_input = self.add_labeled_input(
            self.main_container, "Output directory:", self.default_output_folder
        )
        ...
        self.destructive_checkbox = self.add_checkbox(
            checkbox_container, "Destructive Mode", False
        )
        ...
        self.low_power_checkbox = self.add_checkbox(
            checkbox_container, "Low Power Mode", False
        )
        ...
        self.cutless_checkbox = self.add_checkbox(
            checkbox_container, "Cutless Mode", False
        )
        ...
        detect_button = self.add_button_with_style(
            buttons_container, "Detect", self.detect_commercials, "primary"
        )
        cut_button = self.add_button_with_style(
            buttons_container, "Cut", self.cut_videos, "primary"
        )
        delete_button = self.add_button_with_style(
            buttons_container, "Delete", self.delete_txt_files, "primary"
        )

        self.main_container.append(buttons_container)

        # Continue button to go to the next page
        self.next_button = self.add_button_with_style(
            self.main_container, "Continue", self.on_continue_button_click, "secondary"
        )
    ...
    def on_destructive_mode_changed(self, widget, value=None):
        ...
        if (
            ...
            and hasattr(self, "cutless_checkbox")
            and self.cutless_checkbox.get_value()
        ):
    ...
    def on_fast_mode_changed(self, widget, value=None):
        ...
        if (
            self.fast_mode
            and hasattr(self, "low_power_checkbox")
            and self.low_power_checkbox.get_value()
        ):
    ...
    def on_low_power_mode_changed(self, widget, value=None):
        ...
        if (
            self.low_power_mode
            and hasattr(self, "fast_checkbox")
            and self.fast_checkbox.get_value()
        ):
    ...
    def on_cutless_mode_changed(self, widget, value=None):
        ...
        if (
            self.cutless_mode
            and hasattr(self, "destructive_checkbox")
            and self.destructive_checkbox.get_value()
        ):
    ...
    def update_progress(self, current, total):
        ...
        text_animation_state = (
            "textGlow 2s infinite" if width_percentage > 0 else "none"
        )
    ...
    def update_status(self, text):
        ...
        if any(
            ...
            for keyword in ["processing", "analyzing", "detecting", "cutting"]
        ):
            show_progress = True

        # Use the new method from BasePage with progress indication
        self.update_status_display(
            text, show_progress, self.progress_value if show_progress else None
        )
    ...
    def handle_filtered_files_update(self, filtered_files_json):
        ...
        try:
            ...
            if filtered_files:
                ...
                file_count = len(filtered_files)
                self.update_status_display(
                    f"Received and pre-populated {file_count} filtered files. Switched to file mode."
                )
            else:
                self.update_status_display(
                    "Received empty list of filtered files. Staying in folder mode."
                )
            ...
            print(f"Error processing filtered files: {e}")
            self.update_status_display(
                "An unexpected error occurred while processing filtered files."
            )
...
class Page5(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        self.add_label(
            self.main_container, "What Toonami Version are you making today?"
        )
        options = [
            "Select a Toonami Version",
            "OG",
            "2",
            "3",
            "Mixed",
        ...
        self.channel_number_entry = self.add_labeled_input(
            self.main_container, "What channel number do you want to use?", "e.g., 60"
        )

        # Flex duration entry
        self.flex_duration_entry = self.add_labeled_input(
            self.main_container,
            "Enter your Flex duration Minutes:Seconds (How long should a commercial break be)",
            "e.g., 04:20",
        )

        # Prepare Cut Anime for Lineup button
        self.add_label(self.main_container, "Prepare Cut Anime for Lineup")
        self.prepare_cut_anime_button = self.add_button_with_style(
            self.main_container,
            "Prepare Cut Anime for Lineup",
            self.prepare_cut_anime,
            "primary",
        )

        # Add Special Bumps to Sheet button
        self.add_label(self.main_container, "Add Special Bumps to Sheet")
        self.add_special_bumps_button = self.add_button_with_style(
            self.main_container,
        ...
        self.prepare_plex_button = self.add_button_with_style(
            self.main_container, "Prepare Plex", self.create_prepare_plex, "primary"
        )

        # Create Toonami Channel button
        self.add_label(self.main_container, "Create Toonami Channel")
        self.create_toonami_channel_button = self.add_button_with_style(
            self.main_container,
        ...
        self.add_flex_button = self.add_button_with_style(
            self.main_container, "Add Flex", self.add_flex, "primary"
        )
    ...
    def check_platform_type(self):
        ...
        if platform_type == "tunarr":
            ...
            self.create_toonami_channel_button.set_text(
                "Create Toonami Channel with Flex"
            )
...
class Page6(BasePage):
    def __init__(self, app, *args, **kwargs):
        ...
        self.start_from_last_episode_checkbox = self.add_checkbox(
            self.main_container, "Start from last episode?", default_value=False
        )

        # Prepare Toonami Channel button
        self.add_label(self.main_container, "Prepare Toonami Channel")
        self.prepare_toonami_channel_button = self.add_button_with_style(
            self.main_container,
            "Prepare Toonami Channel",
            self.prepare_toonami_channel,
            "primary",
        )
...
class MainApp(App):
    ...
    def set_current_page(self, page_name):
        if page_name in self.pages:
            ...
            if page_name in ["Page5", "Page6"] and hasattr(
            ...
            if hasattr(self, "logic") and hasattr(
                self.logic, "_broadcast_status_update"
            ):
                status_message = (
                    f"Navigated to {self.page_titles.get(page_name, page_name)}"
                )
    ...
    def go_back(self):
        ...
        if (
            current_page == "Page3"
            and previous_page == "Page2"
            and not self.visited_page2
        ):
        ...
        if current_page == "Page4" or previous_page == "Page4":
            if hasattr(self.pages["Page4"], "cblogic") and hasattr(
                self.pages["Page4"], "set_input_mode"
            ):
